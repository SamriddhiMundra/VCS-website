...
const ModuleDetailPage: React.FC<ModuleDetailPageProps> = ({ moduleNumber, onBack }) => {
    ...
    useEffect(() => {
        ...
        if (moduleNumber) {
            import(`@/data/modules/module${moduleNumber}-content`)
                .then(module => {
                    const contentKey = `module${moduleNumber}Content` as keyof typeof module;
                    setCurriculum(module[contentKey] as ModuleDetail[]);
                })
                .catch(error => {
                    console.error(`Failed to load content for Module ${moduleNumber}:`, error);
                    setCurriculum([]);
                });
        }
    }, [moduleNumber]);
    ...
    if (!curriculum) {
        return (
            <div className="min-h-screen hero-background py-16 px-4 md:px-8 max-w-7xl mx-auto flex flex-col items-center justify-center">
                ...
                <p className="text-white text-lg">Loading Module {moduleNumber} Content...</p>
    ...
    return (
        <div className="min-h-screen hero-background py-16 px-4 md:px-8 max-w-7xl mx-auto">
            ...
            <header className="text-center mb-12">
                <div className="text-sm text-orange-400 font-medium tracking-wider mb-2">Module {moduleInfo.module}</div>
                <h1 className="text-4xl sm:text-5xl font-bold mb-4">{moduleInfo.title}</h1>
                <p className="text-muted-foreground text-base sm:text-lg max-w-4xl mx-auto">{moduleInfo.description}</p>
            </header>

            <section className="mb-12">
                <h2 className="text-2xl font-bold mb-4 text-orange-400 text-center">Key Topics</h2>
                <div className="flex flex-wrap gap-3 justify-center">
                    {moduleInfo.topics.map((topic) => (
                        <span key={topic} className="text-base px-4 py-2 rounded-full bg-orange-500/20 text-orange-200 border border-orange-500/40">
                            {topic}
                        </span>
                    ))}
                </div>
            ...
            <section className="space-y-12">
                <h2 className="text-3xl font-bold text-white text-center">Daily Deep Dive Curriculum</h2>
                
                {curriculum.map((day: ModuleDetail, index) => (
                    <Card key={index} className="p-6 sm:p-8 bg-card/50 backdrop-blur-sm border-border">
                        ...
                        {moduleNumber === 1 && index === 1 && <div className="mt-8 mx-auto max-w-md"></div>}
                    </Card>
                ))}
            </section>
        </div>
...
const CourseClient: React.FC<CourseClientProps> = ({ status }) => {
    ...
    if (activeModule !== null) {
        return <ModuleDetailPage moduleNumber={activeModule} onBack={handleBackClick} />;
    }
